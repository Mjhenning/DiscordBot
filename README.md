...
- **Collaboration Requests** - Create multi-person stream/event proposals with DM invitations, accept/decline flow, and live status tracking. Every participant's DM shows the live roster (statuses, `(you)` marker, external collaborators) while decline reasons stay host-only. Hosts and participants receive persistent DM updates.
...
Full collaboration request pipeline: date picker → modal (title, time, game, external collaborators) → user-select menu → confirmation embed. On confirm, DMs are sent to all participants with Accept/Decline buttons. Responses refresh every DM embed in-place, so each participant sees the live roster of collaborators (status icons, external collaborators) and their own `(you)` marker. On bot boot, all previously sent collab DMs are rebuilt into the current format. Decline reasons are only shown to the host. `/collab view` shows a fox-filtered, paginated schedule-style embed with Next/Previous buttons (hidden on the first/last page). A `CollabCleanupService` sweeps hourly and removes collabs whose date has passed, deleting their DMs. Data persists to `Data/collabs.json`. An in-memory `CollabRequestCache` holds pending requests between modal submission and confirmation.
...
Watches the curated favourites list and posts themed go-live embeds with custom notification messages to a dedicated channel. Instead of EventSub subscriptions (each `stream.online` subscription costs one point of the websocket's 10-point budget), it polls the Get Streams API every minute with the full watchlist in a single call and posts only on an offline-to-live transition. On startup it also sweeps any leftover `stream.online` subscriptions from the old EventSub era so the main notifier keeps its budget. The list is data-driven: `FavouritesData` loads `Data/favourites.json` (seeded with defaults on first run, edit the file to add or remove streamers). The same JSON is read by the Twitch bot (`../DiscordBot/Data/favourites.json`), which auto-shouts out favourited streamers the first time they talk during a stream.
...
Gambling system shared with the TwitchBot via `../../TwitchBot/data/user_data.json` and `network_cache.json`. The Handshake button on the terminal embed opens an ephemeral choice screen with "Unknown Network" and "Other Connection". Unknown Network presents a modal where the user enters an amount of Glossels to gamble. A visible mother number roll (0 to 999) is revealed before each verdict and drives six weighted outcomes: unstable (50%, no change), rejected (16.5%, loss to cache), captured (16.5%, half lost to cache), accepted (11.8%, 2x return), amplified (5%, 3x return), drained (0.2%, drains entire network cache). Other Connection presents a native user picker of your guild; selecting a user who is linked to Twitch opens an amount modal to transfer Glossels, while unlinked users get an error explaining they're not linked yet. Results update the persistent Handshake embed and log to the action history. Requires a linked Twitch-Discord account with a positive Glossel balance. `LinkedAccountsData` re-reads `user_data.json` from disk before every balance lookup and mutating write, and also watches the data directory for the atomic rename the TwitchBot uses, so Glossel balances stay in sync with the TwitchBot without a restart. The `CoherenceWatcher` also watches `network_cache.json`, so the Network Cache shown on the Terminal embed live-updates when the TwitchBot changes it.
...
- **Single EventSub websocket**: The `Twitch_Notifier` service manages the EventSub websocket connection. `TwitchRedeemHandler` and `EventSubReconnectService` share this connection. `FavouritesLiveNoti` does not use it, it polls the REST API instead to stay clear of the websocket subscription budget.
